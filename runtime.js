...
const BASE_URL=(process.env.SARA_PUBLIC_URL||`http://127.0.0.1:${TARGET_PORT}`).replace(/\/$/,'');
const MONITOR_PORT=Number(process.env.RUNTIME_MONITOR_PORT||9090);
...
const monitor=http.createServer((req,res)=>{if(req.url==='/runtime-health'){const ok=lastOk>0&&Date.now()-lastOk<HEARTBEAT_MS*3;res.writeHead(ok?200:503,{'Content-Type':'application/json'});return res.end(JSON.stringify({ok,lastHeartbeat:lastOk?new Date(lastOk).toISOString():null,failures,uptime:process.uptime(),target:BASE_URL}));}res.writeHead(404);res.end('Not found');});
monitor.listen(MONITOR_PORT,()=>console.log(`SARA runtime monitor started on ${MONITOR_PORT}, target ${BASE_URL}`));
